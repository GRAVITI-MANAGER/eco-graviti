...
from .models import Invoice, InvoiceLineItem, Module, Plan, PricingConfig, Subscription, SubscriptionModule, UsageRecord
...
class ModuleAdmin(ModelAdmin):
    ...
    FIELDS_BY_MODULE = {
        "web": {
            ...
            "trial": (("trial_days", "trial_included_ai_requests"),),
            "annual": (("annual_included_ai_requests", "annual_extra_ai_request_price"),),
        },
        "shop": {
            "limits": (
    ...
    def display_module(self, obj):
        base_badge = (
            mark_safe(
                ' <span style="background: #3b82f6; color: white; padding: 1px 6px; border-radius: 4px; font-size: 10px;">BASE</span>'
            )
            if obj.is_base
            else ""
        )
        return format_html("<strong>{}</strong>{}", obj.name, base_badge)
    ...
    def display_price(self, obj):
        ...
        return format_html(
            '<strong>{}{}/mes</strong><br><small class="text-muted">{}{}/año (ahorra {})</small>',
            prefix,
            monthly,
            prefix,
            yearly,
...
class SubscriptionAdmin(ModelAdmin):
    ...
    def display_modules(self, obj):
        ...
        if not modules:
            return format_html('<span style="color: #6b7280;">Solo Web Base</span>')
    ...
    def display_pricing(self, obj):
        ...
        return format_html("<strong>{}/mes</strong>", total)
...
class InvoiceAdmin(ModelAdmin):
    ...
    @display(description="Período")
    def display_period(self, obj):
        return format_html("{} → {}", obj.period_start.strftime("%d/%m/%Y"), obj.period_end.strftime("%d/%m/%Y"))
    ...
    def display_amounts(self, obj):
        ...
        details = " + ".join(parts)
        return format_html('<strong>{}</strong><br><small class="text-muted">{}</small>', total, details)
    ...
    def mark_as_pending(self, request, queryset):
        updated = queryset.filter(status="draft").update(status="pending", issued_at=timezone.now())
