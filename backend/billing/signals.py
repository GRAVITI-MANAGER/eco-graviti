...
def create_subscription_on_tenant_create(sender, instance, created, **kwargs):
    ...
    """
    if created:
        from .models import PricingConfig, Subscription
...
def sync_modules_on_module_change(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.info(f"Módulo {instance.module.name} {action} para {subscription.tenant.name}")
    except Exception as e:
        logger.error(f"Error sincronizando módulo {instance.module.name} para {subscription.tenant.name}: {e}")
...
def sync_modules_on_module_delete(sender, instance, **kwargs):
    ...
    try:
        subscription.sync_modules_to_tenant()
        logger.info(f"Módulo {instance.module.name} eliminado de {subscription.tenant.name}")
    except Exception as e:
        logger.error(f"Error sincronizando tras eliminar módulo {instance.module.name}: {e}")
...
def legacy_sync_plan_modules(sender, instance, created, **kwargs):
    ...
    if hasattr(instance, "plan") and instance.plan:
        try:
            ...
            logger.debug(f"[LEGACY] Módulos sincronizados desde Plan para {instance.tenant.name}")
        except Exception as e:
