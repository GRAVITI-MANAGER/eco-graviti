...
class Promotion(TenantAwareModel):
    ...
    name = models.CharField(
        max_length=300, verbose_name="Nombre de la promoción", help_text="Ej: Dúo Glow: Limpieza + Peeling"
    )

    slug = models.SlugField(max_length=320, blank=True, verbose_name="Slug")

    description = models.TextField(
        blank=True, verbose_name="Descripción", help_text="Descripción detallada de la promoción"
    )
    ...
    promotion_type = models.CharField(
        max_length=20, choices=PROMOTION_TYPES, default="discount", verbose_name="Tipo de promoción"
    )

    # Descuento
    discount_type = models.CharField(
        max_length=20, choices=DISCOUNT_TYPES, default="percentage", verbose_name="Tipo de descuento"
    )
    ...
    # Vigencia
    start_date = models.DateTimeField(verbose_name="Fecha de inicio", help_text="Cuándo comienza la promoción")

    end_date = models.DateTimeField(verbose_name="Fecha de fin", help_text="Cuándo termina la promoción")

    # Visualización
    image = models.ImageField(upload_to="promotions/", null=True, blank=True, verbose_name="Imagen de la promoción")

    badge_text = models.CharField(
        max_length=50, blank=True, verbose_name="Texto del badge", help_text="Ej: '15% OFF', 'OFERTA', '2x1'"
    )
    ...
    is_featured = models.BooleanField(
        default=False, verbose_name="¿Es destacada?", help_text="Mostrar en sección de promociones destacadas"
    )

    priority = models.IntegerField(default=0, verbose_name="Prioridad", help_text="Mayor número = más prioridad")
...
class PromotionItem(TenantAwareModel):
    ...
    promotion = models.ForeignKey(Promotion, on_delete=models.CASCADE, related_name="items", verbose_name="Promoción")
