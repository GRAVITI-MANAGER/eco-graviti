...
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Review.objects.filter(tenant=self.request.tenant).select_related("user").prefetch_related("images")
        ...
        if product_id:
            ...
            queryset = queryset.filter(content_type=content_type, object_id=product_id)
        ...
        if service_id:
            ...
            queryset = queryset.filter(content_type=content_type, object_id=service_id)
    ...
    def helpful(self, request, pk=None):
        ...
        vote, created = ReviewHelpful.objects.get_or_create(tenant=request.tenant, review=review, user=request.user)
        ...
        if not created:
            ...
            review.save()
            return Response({"message": "Voto removido", "helpful_count": review.helpful_count, "has_voted": False})
        else:
            # Nuevo voto
            review.helpful_count += 1
            review.save()
            return Response({"message": "Marcado como útil", "helpful_count": review.helpful_count, "has_voted": True})
    ...
    def my_reviews(self, request):
        ...
        """
        reviews = (
            Review.objects.filter(tenant=request.tenant, user=request.user)
            .select_related("user")
    ...
    def can_review(self, request):
        ...
        if not request.user.is_authenticated:
            return Response({"can_review": False, "reason": "Debes iniciar sesión"})

        product_id = request.query_params.get("product_id")
        service_id = request.query_params.get("service_id")

        if not product_id and not service_id:
            return Response({"error": "Debes especificar product_id o service_id"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_queryset(self):
        ...
        queryset = (
            Review.objects.filter(
                tenant=self.request.tenant, content_type=content_type, object_id=product_id, status="approved"
            )
            .select_related("user")
            .prefetch_related("images")
        )
...
class ServiceReviewsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Review.objects.filter(
                tenant=self.request.tenant, content_type=content_type, object_id=service_id, status="approved"
            )
            .select_related("user")
            .prefetch_related("images")
        )
